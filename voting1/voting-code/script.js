// Constants
const PRIME = 101;
const VALID_EPIC_NUMBERS = ["1234", "6789","432","32","42","45","23","56","789","ST012"];
...
function reconstructSecret(shares) {
    ...
    for (let i = 0; i < shares.length; i++) {
        ...
        secret = (secret + y * numerator * invDenom) % PRIME;
    }

    // Ensure the secret is within the range of party numbers (1 to 5)
    secret = ((secret % 5) + 5) % 5; // Wrap around to ensure it's between 0 and 4
    secret = secret === 0 ? 5 : secret; // Convert 0 to 5 for the party index

    console.log("Final Reconstructed Secret:", secret); // Debugging line
    return secret;
}
...
function submitVerification() {
    ...
    if (share1 && share2 && share3) {
        ...
        // Ensure reconstructed vote is within valid party range (1-5)
        if (reconstructedVote >= 1 && reconstructedVote <= 5) {
            displayVoteVerification(reconstructedVote);
        } else {
            document.getElementById("verifyOutput").innerHTML = `<h3>Verification</h3>
                <p>EPIC Number: ${localStorage.getItem("epicNumber")}</p>
                <p>Invalid Vote</p>`;
        }
...
// Display the verification results based on the reconstructed vote
